...
if (isGroup == true && typeof threadSetting.adminBoxOnly != "undefined") {
    if (threadSetting.adminBoxOnly.status == true && !getDataThread.threadInfo.adminIDs.find(i => i.id == senderID) && !ADMINBOT.includes(senderID) && !OWNER.includes(senderID)) return api.sendMessage("[𝐌𝐨𝐝𝐞] » Chỉ admin nhóm được sử dụng lệnh", threadID, messageID);
}
    ...
    if (checker.bestMatch.rating >= 0.5) command = client.commands.get(checker.bestMatch.target);
      ...
      let link = data.nino[Math.floor(Math.random() * data.nino.length)].trim()
